...
class InternetExchange(InternetExchangeBase):
    ...
    @property
    def fac_set(self):
        return [ixfac.fac for ixfac in self.ixfac_set.all()]

    @property
    def lans(self):
        return [ixlan.id for ixlan in self.ixlan_set.all()]

    @property
    def prefixes(self):
        return [prefix.prefix.compressed for prefix in IXLanPrefix.objects.filter(ixlan__ix__id=self.id)]
